    ...
    <section id="process" className="py-20 md:py-28 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        <div ref={ref} className="reveal mb-16">
          <p className="font-mono text-cyan-400 text-sm tracking-widest uppercase mb-3">Process</p>
...
function ProcessStep({ num, label, title, description, index }) {
  ...
  return (
    ...
    >
      ...
      <div className="hidden lg:block absolute top-6 left-[calc(100%+4px)] w-[calc(100%-32px)] h-px border-t border-dashed border-cyan-600/50" />

      <div className="flex items-center gap-3 mb-4">
        <span className="font-mono text-sm text-cyan-400 bg-cyan-600/10 px-2 py-0.5 rounded">{label}</span>
        <span className="font-mono text-3xl font-bold text-cyan-600/40" aria-hidden="true">
          {num}
        </span>
      </div>
